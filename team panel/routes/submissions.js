...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const allowedTypes = /jpeg|jpg|png|gif|pdf|doc|docx|ppt|pptx|xls|xlsx|zip|rar|txt|md|py|js|html|css|java|cpp|c|php|sql/;
...
router.post('/upload/:teamId/:round', upload.array('files', 10), async (req, res) => {
  try {
    const team = await Team.findOne({ teamId: req.params.teamId });
    if (!team) {
      return res.status(404).json({ message: 'Team not found' });
    ...
    io.to(`team-${team._id}`).emit('submission-uploaded', { team, submission });
...
router.get('/download/:teamId/:round/:filename', async (req, res) => {
  try {
    ...
    const submission = team.submissions.find(sub => sub.round === req.params.round);
    if (!submission) {
      return res.status(404).json({ message: 'Submission not found' });
    }

    const file = submission.files.find(f => f.filename === req.params.filename);
    if (!file) {
...
router.put('/status/:teamId/:round', async (req, res) => {
  try {
    ...
    // Emit real-time update
    const io = req.app.get('io');
    io.to(`team-${team._id}`).emit('submission-status-updated', { team, submission });
...
router.delete('/file/:teamId/:round/:filename', async (req, res) => {
  try {
    ...
    const fileIndex = submission.files.findIndex(f => f.filename === req.params.filename);
    if (fileIndex === -1) {
      return res.status(404).json({ message: 'File not found' });
    }
    ...
    // Emit real-time update
    const io = req.app.get('io');
    io.to(`team-${team._id}`).emit('submission-file-deleted', { team, submission });
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const stats = {
      totalSubmissions: 0,
      submissionsByRound: {
        IST: { total: 0, uploaded: 0, reviewed: 0, qualified: 0, notQualified: 0 },
        'Round 1': { total: 0, uploaded: 0, reviewed: 0, qualified: 0, notQualified: 0 },
        'Round 2': { total: 0, uploaded: 0, reviewed: 0, qualified: 0, notQualified: 0 }
      },
    ...
    stats.averageFilesPerSubmission = stats.totalSubmissions > 0 
      ? totalFiles / stats.totalSubmissions 
      : 0;
